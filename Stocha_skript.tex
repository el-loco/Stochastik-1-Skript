...
z\mapsto\int e^{\langle z,T\rangle} hd\mu=:\xi(z)\]
beliebig oft differenzierbar ist und Integration und Differentiation vertauscht werden können.\\

\underline{Intuition:} $hd\mu=dp$ Wahrscheinlichkeitsmaß und $P^T$ die Verteilung von T unter $P^T$ darstellt. Dann \[
\int\limits_\omega e^{\langle z,T\rangle}P^T(dt)=\varphi_T(z)\quad z\in\C\]
Momente bis zur Ordnung $\gamma$ existieren für $T_j$ falls\hfill(*)\\ $T\in\lp{\gamma}(P)$.\\
...
$\tilde{C}(S))\dfrac{\partial^{\mu_1+...+\mu_k}}{{\partial s_1^{\mu_1}\partial s_2^{\mu_2}...\partial s_k^{\mu_k}}}\int\limits_\Omega e^{\underbrace{\langle s,T\rangle}_{\sm[k][{i=1}s_iT_i}}d\mu=\int\limits_\Omega\prod\limits_{j=1}^kT_j^{\mu_j}\underbrace{e^{\langle s,T\rangle}\tilde{C}(s)h(x)d\mu(x)}_{dQs}$
\item Es gilt $\tilde{C}(s)=\dfrac{1}{\xi(s)}\\
\Ra\dfrac{\partial}{\partial s_i}\tilde{C}(s)=-\dfrac{1}{\beta (s)}\circ\dfrac{\partial}{\partial s_i}\beta(s)=-\dfrac{1}{\beta(s)}\circ\int\limits_\Omega T_ie^{\langle s,T\rangle}h(x)d\mu(x)\\
=-\dfrac{1}{\beta(s)}\underbrace{\int\limits_\Omega T_i(x)\underbrace{e^{\langle s,T(x)\rangle}\tilde{C}(s)h(x)d\mu(x)}_{dQs}}_{E_{Q_s}(T_i)}$
...
und damit\\
$\dfrac{\partial}{\partial s_i} E_{Q_s}(\varphi)..........$
\end{enumerate}


\subsection{Definition}
\label{11.12}
Sei $\varphi$ ein Maß auf $\R^k$, dann heißt\[
L_\varphi:y\mapsto\int\limits_{\R^k}e^{\langle y,x\rangle} d\varphi(x), \R^k\mapsto [0,\infty]\]
die Laplacetransformierte oder momenterzeugende Funktion von $\varphi$. $I_\varphi:=\brac{y\in\R^k:L_\varphi(y)<\infty}$

\subsection{Satz (Eindeutigkeitssatz)}
\label{11.13}
Seien $\varphi_1,\varphi_2$ Maße auf $\R^k$ und $\mc{U}\neq\emptyset$ offene Menge mit $\mc{U}\subset I_{\varphi_1}$ und $\mc{U}\subset I_{\varphi_2}.$ Gilt \[
L_{\varphi_1}(y)=L_{\varphi_2}(y)\qf y\in\mc{U}\]
...
\subsection{Satz}
\label{11.14}
Sei $\p$ eine Exponentialfamilie. Hat der Parameterraum $\Xi=\brac{(g_1(\varphi),...,g_k(\varphi)):\vartheta\in\theta}\subseteq\R^k$ nichtleeres Inneres, d.h. $\Xi\neq\emptyset$, dann ist $T=(T_1,...,T_k)$ vollständig für $\p$\\
\beweis
...
z.z. nach \hyperref[11.6]{Bemerkung 11.6 (i)}: $f\circ T=0\quad P\vartheta-f.s.\qf\vartheta\\
...
$\Ra f=0\quad\mu^T-$f.ü $\Ra f\circ T=0\mu-$f.ü.\\
...
\begin{enumerate}[label=(\roman*)]
\item $T=\sumi x_i $ ist suffizient und vollständig für $P_i$ mit $i=\underbrace{2}_{\substack{vgl. 10.15.c)\\Vollst.vgl.ii}}\overbrace{4}^{\substack{einfach \\zu sehen}}\underbrace{5}_{11.7 b)}$\\
Mit Lehmann-Scheffe ist dann \[
h^*=\dfrac{1}{n}\sumi x_i=\overline{x}_n=\dfrac{1}{n}E\brc{\sumi x_i|\sigma(T)}=E(\overleftarrow{x}_n|\sigma(T))\]
ist UMVU Schätzer für \[
g(\varphi)=\int xdP\varphi(x)=E_\varphi(x_1)\]
(UMVUE ist f.s. eindeutig)
\item $T=\brc{\sumi x_i,\sumi x_i^2}$ ist suffizient für $\p_1$ (\hyperref{Bsp. 10.15 c)} und dort $\\
\dfrac{dP_\varphi}{d\lambda^n}=C(\varphi)\exp\brc{\underbrace{\dfrac{\mu}{\sigma^2}}_{g_1(\mu\sigma^2}\underbrace{\sumi x_i}_{T_1(x)}-\underbrace{\dfrac{1}{2\sigma^2}}_{g_2(\mu,\sigma^2}\underbrace{\sumi x_i^2}_{T_2(x)}}\\
\Ra\Xi=\brac{(g_1,(\mu,\sigma^2),g_2(\mu,\sigma^2),g_2(\mu,\sigma^2)):\mu\in\R,\sigma^2>0}\\
=\brac{(\dfrac{\mu}{\sigma^2},-\dfrac{1}{2\sigma^2}:\mu\in\R,\sigma^2>0}\\
=\R x(-\infty,0)\neq\emptyset$\\
also mit \hyperref[11.14]{Satz 11.14} gilt $T=\brc{\sumi x_i,\sumi x_i^2}$ vollständig für $\p_1$.\[
\widehat{\mu}_n=\dfrac{1}{n}\sumi x_i, \widehat{\sigma}_n^2=\dfrac{1}{n-1}\sumi(x_i-n\widehat{\mu}_n)^2\] Beide erwartungstreu und mb bzgl $\sigma(T)$ .\\
...
\tilde{P}:=\brac{\otimes\limits_{i=1}^nP_i:P_i\in\p_i,i=1,\cdots,n}\]
Dann ist \[
\brc{\prod\limits_{i=1}^n\Omega_i,\otimes\limits_{i=1}^n\f_i,\tilde{\p}\]
vollständig.
\item Sei $\p\subseteq M_1\rbor[]$ und $(\R,\mc{B}(\R),\p)$ sei vollständiges SE.\\
...
\fi
\marginpar{20.Vorlesung, 09.07.2018}
...
P^V=Q\qf P\in\p\]
für ein Wahrscheinlichkeitsmaß $Q$ auf $(\Omega'',\f'')$. Dann sind T und V P-stochastisch unabhängig $\forall P \in\p$\\
\beweis 
Sei $A\in\f''$. \[
E_P(\underbrace{E_\bullet(1_A(V)|T)-Q(A)}_{=:f})=E_P(1_A(V))-Q(A)=P^V(A)-Q(A)=0\]
Da f beschränkt ist, gilt $f=0\ P-f.s.$ d.h. \[
Q(A)=E_\bullet(1_A(V)|T)\qf A\in\f''\]
Sei nun $B\in\f'$. Dann gilt für $P\in\p\\
P(V\in A,T\in B)=E_P(1_A(V)1_B(T)=E_P(E_P(1_A(V)1_B(T)|T))=E_P(1_B(T)E_\bullet(1_A(V)|T))=Q(A)P(T^{-1}(B))=P(V\in A)P(T\in B)$\\
...
\subsection{Annahme}
\label{13.1}
$\rraum$ sei ein SE mit $\p=\brac{P_\vartheta:\vartheta\in\theta}\ll\mu$ für $\mu \sigma$-endlich, wobei $\theta\subseteq\R$ offen ist und für $f_\vartheta:=\dfrac{dP_\vartheta}{d\mu}$ gilt:\begin{list}{•}{}
...
\item In a) und b) gilt \glqq $=$ \grqq$\LRa \cdot l_\vartheta=A(\vartheta)(T-E_\vartheta(T))\quad\mu-$f.ü. mit einer Funktion A
\item Ist $\vartheta\mapsto\int f_\vartheta d\mu$ 2-fach differenzierbar unter dem Integral, so gilt:\[
I_\vartheta=-E_\vartheta\brc{\dfrac{\partial^2}{\partial\vartheta^2}log(f_\vartheta)}\]
\end{enumerate}
\beweis
$g(\vartheta)+b(\vartheta)=\int Tf_\vartheta d\mu=\int\limits_{B^c}Tf_\vartheta(d\mu)\\
\Ra \dot{g}(\vartheta)+\dot b(\vartheta)=\int\limits_{A^c\cap B^c}T\underbrace{\dfrac{\partial}{\partial\vartheta}f_\vartheta}_{\dfrac{\dot f_\vartheta}{f_\vartheta}f\vartheta}=\int\limits_{A^c\cap B^c} T\dot l_\vartheta \underbrace{f_\vartheta d\mu}_{dP_\vartheta}\\
...
\Ra 0=\int\dfrac{\partial^2}{\partial\vartheta^2}d_\vartheta d\mu\xlongequal[regel]{Produkt-}\int\ddot l_\vartheta f_\vartheta d\mu+\int \brc{\dot l_\vartheta}^2\underbrace{f_\vartheta d\mu}_{dP_\vartheta}\\
...
\label{13.4}
\begin{enumerate}[label=(\roman*)]
\item Die rechten Seiten der Ungleichungen in a) und b) heißen auch Informaionsschranken
\item Die Craner-Rao-Ungleichung lässt sich auch unter weicheren Voraussetzungen beweisen (\grqq$\lp{1}$-Differenzierbarkeit\grqq)
\end{enumerate}
